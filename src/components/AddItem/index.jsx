...
function AddItem({onAdd}) {
    const [isVisbile, setVisible] = React.useState(false);
    const [inputTitleValue, setInputTitleValue] = React.useState('');
    const [inputColorValue, setInputColorValue] = React.useState('#ffffff');

    document.onclick = (event) => {
        if (!event.target.closest('.addList-popup') && !event.target.closest('.todo__add-item')) {
            setVisible(false);
        }
    };
    ...
    const closePopup = () => {
        setInputTitleValue('');
        setInputColorValue('#ffffff');
        setVisible(false);
    };


    return (
        <div className="todo__add-item-wrap">
            <button className="todo__add-item" onClick={() => setVisible(true)}>
                <i>
            ...
            <div className={isVisbile ? "addList-popup active" : "addList-popup"}>
                <div className="addList-popup__close" onClick={() => setVisible(false)}>
                    <i>
                ...
                <div className="addList-popup__input-name">
                    <input value={inputTitleValue} onChange={e => setInputTitleValue(e.target.value)} type="text" placeholder="Название папки"/>
                </div>

                <div className="addList-popup__select-color">
                    <input value={inputColorValue} onChange={e => setInputColorValue(e.target.value)} type="color" id="colorSelect"/>
                    <label htmlFor="colorSelect">Выбрать цвет</label>
                </div>

                <button className="addList-popup__add-folder" onClick={addItem}>Добавить</button>
            </div>
        </div>
    )
}
